...
# ------------------------------------------------------------
# Helper: construye la cadena de tooltip (Fecha + etiqueta + valor + unidad)
# ------------------------------------------------------------
crear_tooltip <- function(fechas, valores, etiqueta, unidad) {
  vals_fmt <- ifelse(is.na(valores), "s/d", formatC(valores, format = "f", digits = 2))
  paste0("Fecha: ", format(fechas, "%d/%m/%Y"),
         "<br>", etiqueta, ": ", vals_fmt, " ", unidad)
}
...
graficar_dyn <- function(df_merge, anios_p1, anios_p2, titulo_hist, titulo_anom,
  ...
  df_plot$tooltip_datos <- crear_tooltip(df_plot$fechas, df_plot$datos,    "Datos",           unidad)
  df_plot$tooltip_anom  <- crear_tooltip(df_plot$fechas, df_plot$anomalia, "Anomalía",        unidad)
  df_clim24$fecha_tip   <- as.Date("2021-01-01") + (df_clim24$dia_rel - 1)
  df_clim24$tooltip_clim <- crear_tooltip(df_clim24$fecha_tip, df_clim24$clim_dia,
                                          "Prom. Histórico", unidad)
  ...
  p_hist <- ggplot() +
    geom_segment(data=df_clim24,
                 aes(x=dia_rel, xend=dia_rel, y=ymin, yend=clim_dia,
                     color="Prom. Histórico", text=tooltip_clim, group=1),
                 linewidth=0.37, alpha=0.8) +
    geom_vline(xintercept=366, color=c_sep, linetype="dotted", linewidth=0.5, alpha=0.7) +
    geom_line(data=df_plot,
              aes(x=dia_rel, y=datos, color=Serie, text=tooltip_datos, group=Serie),
              linewidth=0.5) +
  ...
  p_anom <- ggplot(df_plot, aes(x=dia_rel, y=anomalia, color=Serie, text=tooltip_anom, group=Serie)) +
    geom_hline(yintercept=0, color=c_cero, linetype="dashed", linewidth=0.5) +
    geom_line(linewidth=0.5) + scale_color_manual(name=NULL, values=colores2) +
    scale_x_continuous(breaks=eje$dias, labels=eje$meses, limits=c(1,731), expand=c(0.01,0)) +
...
graficar_sst <- function(df_merge, anios_p1, anios_p2, titulo_hist, titulo_anom,
  ...
  p_hist <- ggplot() +
    ...
    geom_line(data=df_clim24,
              aes(x=dia_rel, y=clim_dia, color="Prom. Histórico", text=tooltip_clim, group=1),
              linewidth=1) +
    geom_line(data=df_plot,
              aes(x=dia_rel, y=datos, color=Serie, text=tooltip_datos, group=Serie),
              linewidth=0.5) +
...
# ------------------------------------------------------------
# graficar_w: barras horizontales + tooltips personalizados
# Devuelve 4 plots: hist_p1, hist_p2, anom_p1, anom_p2
# ------------------------------------------------------------
graficar_w <- function(df_merge, anios_p1, anios_p2, titulo_hist, titulo_anom,
                       ylab_hist = "Velocidad [m/s]", ylab_anom = "Anomalía [m/s]",
                       colores = NULL, tema = "bw") {
  ...
  label_p1 <- paste0("Años: ", anios_p1[1], "-", anios_p1[2])
  label_p2 <- paste0("Años: ", anios_p2[1], "-", anios_p2[2])
  
  if (nrow(df_p1) == 0 && nrow(df_p2) == 0) {
    ...
    return(list(hist_p1 = p_aviso, hist_p2 = p_aviso,
                anom_p1 = p_aviso, anom_p2 = p_aviso))
  }
  
  agregar_tooltips <- function(df) {
    if (nrow(df) == 0) {
      df$tooltip_datos <- character(0)
      df$tooltip_anom  <- character(0)
      df$tooltip_clim  <- character(0)
      return(df)
    }
    df$tooltip_datos <- crear_tooltip(df$fechas, df$datos,     "Datos",           unidad)
    df$tooltip_anom  <- crear_tooltip(df$fechas, df$anomalia,  "Anomalía",        unidad)
    df$tooltip_clim  <- crear_tooltip(df$fechas, df$clim_dia,  "Prom. Histórico", unidad)
    df
  }
  df_p1 <- agregar_tooltips(df_p1)
  df_p2 <- agregar_tooltips(df_p2)
  
  if (nrow(df_p1) > 0)
    df_p1 <- df_p1 %>% dplyr::mutate(Color_Anom = ifelse(anomalia >= 0, "Positiva", "Negativa"))
  if (nrow(df_p2) > 0)
    df_p2 <- df_p2 %>% dplyr::mutate(Color_Anom = ifelse(anomalia >= 0, "Positiva", "Negativa"))
  
  datos_all <- dplyr::bind_rows(df_p1, df_p2)
  
  max_abs_h <- max(abs(c(datos_all$datos, datos_all$clim_dia)), na.rm = TRUE)
  if (!is.finite(max_abs_h) || max_abs_h == 0) max_abs_h <- 10
  xmax_h <- ceiling(max_abs_h)
  
  max_abs_a <- max(abs(datos_all$anomalia), na.rm = TRUE)
  ...
  theme_extra <- theme(
    ...
    plot.subtitle    = element_text(hjust = 0.5, size = 12, color = "#475569", margin = margin(b = 8)),
    panel.grid.minor = element_blank(),
  ...
  crear_hist <- function(df, label) {
    if (is.null(df) || nrow(df) == 0) return(ggplot_aviso("Datos insuficientes", label))
    ggplot(df) +
      geom_vline(xintercept = 0, color = "black", linewidth = 0.3) +
      geom_segment(aes(y=dia_rel, yend=dia_rel, x=0, xend=datos,
                       color=label, text=tooltip_datos, group=1),
                   linewidth = 0.37) +
      geom_path(aes(y=dia_rel, x=clim_dia, color="Prom. Histórico",
                    text=tooltip_clim, group=1),
                linewidth = 0.5) +
      geom_hline(yintercept = 366, color = c_sep, linetype = "dashed",
                 linewidth = 0.5, alpha = 0.7) +
      scale_color_manual(name = NULL,
                         values = setNames(c(c_hist_bar, c_hist_line),
                                           c(label, "Prom. Histórico"))) +
      ...
      labs(title = titulo_hist, subtitle = label, x = ylab_hist, y = NULL) +
      tema_obj + theme_extra +
      theme(legend.position = "bottom", legend.justification = "right", legend.key = element_blank())
  }
  
  crear_anom <- function(df, label) {
    if (is.null(df) || nrow(df) == 0) return(ggplot_aviso("Datos insuficientes", label))
    ggplot(df) +
      geom_vline(xintercept = 0, color = "black", linewidth = 0.3) +
      geom_segment(aes(y=dia_rel, yend=dia_rel, x=0, xend=anomalia,
                       color=Color_Anom, text=tooltip_anom, group=1),
                   linewidth = 0.37) +
      geom_hline(yintercept = 366, color = c_sep, linetype = "dashed",
                 linewidth = 0.5, alpha = 0.7) +
      ...
      scale_y_reverse(breaks = eje$dias, labels = eje$meses, expand = c(0.01, 0)) +
      labs(title = titulo_anom, subtitle = label, x = ylab_anom, y = NULL) +
      tema_obj + theme_extra +
      theme(legend.position = "none")
  }
  
  list(
    hist_p1 = crear_hist(df_p1, label_p1),
    hist_p2 = crear_hist(df_p2, label_p2),
    anom_p1 = crear_anom(df_p1, label_p1),
    anom_p2 = crear_anom(df_p2, label_p2)
  )
}
...
construir_plots_contorno_t <- function(datos, titulo_hist, titulo_anom,
  ...
  crear_plot_temp <- function(df, titulo) {
    ...
    ggplot(df, aes(x = DIA_AÑO, y = Profundidad)) +
      ...
      geom_text_contour(aes(z = TEMP), breaks = datos$breaks_solid, size = 4.5, rotate = TRUE, min.size = 2, skip = 1, fontface = "bold", label.placer = label_placer_n(1)) +
      geom_text_contour(aes(z = TEMP), breaks = datos$breaks_dashed, size = 4.5, rotate = TRUE, min.size = 2, skip = 1, label.placer = label_placer_n(1), fontface = "plain", color = c_label) +
  ...
  crear_plot_anom <- function(df, titulo) {
    ...
    ggplot(df, aes(x = DIA_AÑO, y = Profundidad)) +
      ...
      geom_contour(aes(z = ANOM), breaks = datos$breaks_neg_a, color = c_anom_dash, linewidth = 0.5, linetype = "dashed") +
      geom_text_contour(aes(z = ANOM), breaks = datos$breaks_pos_a, size = 4.5, rotate = TRUE, min.size = 1, skip = 1, fontface = "bold", label.placer = label_placer_n(1), check_overlap = FALSE) +
      geom_text_contour(aes(z = ANOM), breaks = datos$breaks_neg_a, size = 4.5, rotate = TRUE, min.size = 1, skip = 1, label.placer = label_placer_n(1), fontface = "plain", color = c_label, check_overlap = FALSE) +
