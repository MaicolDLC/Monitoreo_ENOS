...
# ------------------------------------------------------------
# CORRECCIÓN:
#   - Se reemplaza el tema "Oscuro" (theme_dark, fondo gris oscuro)
#     por "Gris (defecto ggplot)" (theme_gray, fondo gris claro #EBEBEB).
#   - Se agregan comentarios defensivos: el selector de tema SOLO
#     se ofrece a paneles de LÍNEAS (sst, dyn, iso) y NUNCA a los
#     paneles de barras (w) ni de contornos (t).
# ============================================================
...
css_monitoreo <- HTML("
  ...
  /* Estilo grid para los 5 color inputs */
  ...
  /* Botón compacto de descarga en card header */
...
# Devuelve el objeto theme() de ggplot2 según el nombre.
# ------------------------------------------------------------------
# IMPORTANTE: esta función SOLO debe ser llamada por las gráficas de
# LÍNEAS (graficar_sst, graficar_dyn, graficar_iso). NUNCA por
# graficar_w (barras / segmentos) ni por construir_plots_contorno_t,
# los cuales usan theme_bw() fijo por diseño.
# ------------------------------------------------------------------
obtener_tema <- function(nombre = "minimal") {
  switch(nombre,
         ...
         "gray"    = ggplot2::theme_gray(),   # <-- fondo gris claro #EBEBEB (defecto ggplot)
         ggplot2::theme_minimal()
  )
}

# Nombres legibles de las 5 posiciones de color por variable
...
# Colores por defecto (coinciden con los hardcoded originales)
...
# Constructor de los 5 colourInput
...
# Constructor del selector de tema.
# ------------------------------------------------------------------
# Esta función SOLO se llama desde crear_tab_variable() cuando
# var_id %in% c("sst", "dyn", "iso"). En "w" (barras) y "t"
# (contornos) el panel de apariencia se omite por completo.
# ------------------------------------------------------------------
...
# Botón compacto de descarga para el card header
boton_descarga <- function(id) {
  downloadButton(id, NULL,
                 icon = shiny::icon("download"),
                 class = "btn-dl-header",
                 title = "Descargar PNG alta resolución (300 DPI)")
}