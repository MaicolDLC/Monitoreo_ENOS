...
library(colourpicker)   # <-- NUEVO: selectores de color
...
crear_tab_variable <- function(var_id, titulo) {
  ...
  # --- NUEVO: Panel de colores (5 por variable) ---
  ...
  # --- NUEVO: Panel de tema SOLO para gráficas de línea ---
  panel_tema <- if (var_id %in% c("sst", "dyn", "iso")) {
    accordion_panel("Apariencia", icon = bsicons::bs_icon("brush-fill"),
                    crear_control_tema(var_id))
  } else NULL
  
  # --- Renderizado de tarjetas con botón de descarga ---
  if (var_id == "t") {
    ...
    altura_grafica <- "520px"
    spinner_hist <- withSpinner(plotOutput(paste0("hist_", var_id), height = altura_grafica),
                                type = 8, color = "#0284C7", size = 1.2, color.background = "#FFFFFF")
    spinner_anom <- withSpinner(plotOutput(paste0("anom_", var_id), height = altura_grafica),
                                type = 8, color = "#0284C7", size = 1.2, color.background = "#FFFFFF")
    ...
    contenido_graficas <- if (var_id == "w") layout_columns(col_widths = c(6, 6), card_hist, card_anom)
    else div(card_hist, card_anom)
  }
            ...
            layout_sidebar(
              sidebar = sidebar(width = 340,
                                ...
                                accordion(multiple = TRUE, open = c("Boya", "Períodos"),
                                          ...
                                          extra_panel_t,
                                          panel_colores,        # NUEVO
                                          panel_tema,           # NUEVO (solo sst/dyn/iso)
...
server <- function(input, output, session) {
  ...
  # ============================================================
  # GRÁFICAS POR PESTAÑA (sst, dyn, iso, w)
  # ============================================================
  ...
  for (v in names(variables_config)) {
    local({
      ...
      # NUEVO: colores (sin debounce para feedback inmediato)
      ...
      # NUEVO: tema (solo existe en sst/dyn/iso; para w devolvemos "minimal")
      tema_react <- reactive({
        if (var_id %in% c("sst", "dyn", "iso")) {
          input[[paste0("tema_", var_id)]] %||% "minimal"
        } else "minimal"
      })
      ...
      grafico_react <- reactive({
        ...
        tryCatch({
          ...
          if (var_id %in% c("sst", "dyn", "iso")) args_list$tema <- tema_react()
      ...
      output[[id_hist_out]] <- renderPlot({ grafico_react()$hist })
      output[[id_anom_out]] <- renderPlot({ grafico_react()$anom })
      
      # ---- NUEVO: Download handlers PNG 300 DPI ----
      ancho_dl <- 14; alto_dl <- 7
      
      output[[paste0("dl_hist_", var_id)]] <- downloadHandler(
        filename = function() paste0("hist_", var_id, "_", Sys.Date(), ".png"),
        content = function(file) {
          g <- grafico_react()$hist
          ggsave(file, plot = g, width = ancho_dl, height = alto_dl,
  ...
  # ============================================================
  # Bloque específico para Temperatura Subsuperficial (4 filas)
  # ============================================================
  local({
    id_boya_in <- "boya_t"
    boya_sel_t <- reactive({
    ...
    # ---- NUEVO: Descargas de los 4 contornos ----
    output$dl_hist_p1_t <- downloadHandler(
      ...
      content = function(file) {
        ggsave(file, plot = plots_contorno_react()$hist_p1,
               width = 14, height = 10, dpi = 300, bg = "white", units = "in", limitsize = FALSE)
      }
    )
